#Functions for the iteration of the algorithm for Elaboo
...
def tree_iterate_all(alignment_list, directory, boots = 100, model = None, data_type = "PROT", method = "fast"):
	...
	for aln in alignment_list:
		...
		if method == "fast":
			fasttreecmd = Applications._Fasttree.FastTreeCommandline(input = aln, out = directory+"/ft_"+str(count)+".tre")
			logging.info("FastTree called as: %s" % fasttreecmd)
			fasttreecmd()
			logging.info("FastTree run finalized")
		elif method == "rax":
			seed = random.randint(10000,99999)
			raxml_obj = Applications.RaxmlCommandline(sequences = aln, model = model, name=namestr, num_replicates = boots, working_dir = os.getcwd()+'/'+directory+'/', bootstrap_seed = seed)
...
def tree_placement(alignment, prob_taxa, model = None, reps = 5, keep = False, temporal_dir = None):
	...
	if model is None:
		...
		evo_model = model
	seed = random.randint(10000,99999)
	raxml_obj = Applications.RaxmlCommandline(sequences = namepath+"base_alignment.phy", model = evo_model, name="basetree", num_replicates = reps, working_dir = namepath, parsimony_seed = seed)
	logging.info("RAxML called as: %s" % raxml_obj)
	raxml_obj()
	logging.info("RAxML run finalized")
	if not keep:
	...
	seed = random.randint(10000,99999)
	raxml_epa_obj = Applications.RaxmlCommandline(sequences = namepath+"clean.phy", model = evo_model, name="EPA.tre", working_dir = namepath, algorithm = 'v', starting_tree = namepath+"RAxML_bestTree.basetree")
